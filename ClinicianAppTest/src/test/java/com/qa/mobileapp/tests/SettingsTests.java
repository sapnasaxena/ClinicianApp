...
public class SettingsTests extends TestBase {
	...
	@Test(groups = { "smoketest" })
	public void TC_03_testVerifyHelpScreenVisibilty()
	{
		System.out.println("Verify that on clicking on Help,Help screen should be visible");
        dashboardpage.onTapDashboardtext();
		GlobalUtil.wait(2);
		dashboardpage.onTapSettingsExpandButton();
		GlobalUtil.wait(2);
		settingspage.onClickHelp();
		GlobalUtil.wait(2);
		Assert.assertNotNull(settingspage.getHelpScreenLocator());
	}
	@Test(groups = { "smokeTest" })
	public void TC_03_testVerifyAboutAppScreenVisibilty()
	{
		System.out.println("Verify that on clicking on Help,Help screen should be visible");
        dashboardpage.onTapDashboardtext();
		GlobalUtil.wait(2);
		dashboardpage.onTapSettingsExpandButton();
		GlobalUtil.wait(2);
		settingspage.onClickAboutApp();
		GlobalUtil.wait(2);
		Assert.assertNotNull(settingspage.getAboutAppScreenLocator());
	}
